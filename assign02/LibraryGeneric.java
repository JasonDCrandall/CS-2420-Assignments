...
/**
 * This class represents a generic library, which is a collection of generic library books.
 * 
...
public class LibraryGeneric<Type> {
		...
		for (LibraryBookGeneric<Type> book : library)
			if (isbn == book.getIsbn())
				if (book.checkAvailability()) {
					...
					book.setDueDate(year, month, day);
					return true;
				}
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day, int year) {
		...
		ArrayList<LibraryBookGeneric<Type>> libraryOverdueCopy = new ArrayList<LibraryBookGeneric<Type>>();
		
		// If no books are in the library, return an empty array list
		if (library.isEmpty())
			return libraryOverdueCopy;
		
		// Searches through the library and adds overdue books to the array list
		for (LibraryBookGeneric<Type> book : library) { 
			if (book.getDueDate() == null)
				continue;
			int compare = book.getDueDate().compareTo(inputDate);
			if (compare < 0)
				libraryOverdueCopy.add(book);		
		}
		
		OrderByDueDate comparator = new OrderByDueDate();

		sort(libraryOverdueCopy, comparator);
	...
	/**
	 * Comparator that defines ordering among library books in lexicographical order by title.
	 */
